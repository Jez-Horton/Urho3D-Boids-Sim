...
{
	///Creating the boid Nodes
	boidNode = pScene->CreateChild("boid");
	boidNode->SetPosition(Vector3(0.0f, 10.0f, 50.0f));
	boidNode->SetRotation(Quaternion(0.0f, 0.0f, 0.0f));
	boidNode->SetScale(1.0f);
	///Setting the models, textures and meshes for the boids
	pObject = boidNode->CreateComponent<StaticModel>();
	...
	//Putting a RigidBody on each boid, this is the collider needed to calculate the players health
	...
	pCollisionShape = boidNode->CreateComponent<CollisionShape>();
